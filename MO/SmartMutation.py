...
def randomswap(problem, oldroutes):
    routes = oldroutes
...
def timingswap(problem, oldroutes):
    routes = oldroutes
    pickup1 = random.randint(1, problem.n)  # randomly pick 2 requests
    pickup2 = random.randint(1, problem.n)
    dropoff1 = pickup1 + problem.n
...
def distanceswap(problem, oldroutes):
    routes = oldroutes
    pickup1 = random.randint(1, problem.n)  # randomly pick 2 requests
    pickup2 = random.randint(1, problem.n)
    dropoff1 = pickup1 + problem.n
...
def durationswap(problem, oldroutes):
    routes = oldroutes
    pickup1 = random.randint(1, problem.n)  # randomly pick 2 requests
    pickup2 = random.randint(1, problem.n)
    dropoff1 = pickup1 + problem.n
...
class SmartMutation(Mutation):
    ...
    def _do(self, problem, X, **kwargs):
        ...
        for i in range(n_individuals):
            ...
            if rng <= 25:
                newroutes = randomswap(problem, routes)
            elif rng <= 50:
                newroutes = timingswap(problem, routes)
            elif rng <= 75:
                newroutes = distanceswap(problem, routes)
            else:
                newroutes = durationswap(problem, routes)

            individual = problem.matrixtooutput(problem.routetomatrix(newroutes))
            X[i, :] = individual
